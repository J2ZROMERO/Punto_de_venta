...
import interfaces.Producto.ProductoUnitario;
import metodos_personalizados.Metodos_numericos;
import metodos_personalizados.SoloDecimalTextField;
...
if(tbl_ventas.getRowCount() != 0 && tbl_ventas.getSelectedRow() != -1) {
					...
					String comparar_solicitud = def_tabla.getValueAt(tbl_ventas.getSelectedRow(), 5).toString(); 
					double stock = Double.parseDouble(def_tabla.getValueAt(tbl_ventas.getSelectedRow(),3).toString());
					double totalVenta = Double.parseDouble(def_tabla.getValueAt(tbl_ventas.getSelectedRow(), 5).toString());
					
					if(comparar_solicitud.contains(".")) {
						String suma = Metodos_numericos.convierteAkilogramos(totalVenta+=.100);
						
						
						System.out.println(Double.parseDouble(suma));
						if(totalVenta < stock) {
						...
						int cantidadSolicitada = Integer.parseInt(def_tabla.getValueAt(tbl_ventas.getSelectedRow(), 5).toString());
						...
						for(int i = 0; i <  def_tabla.getRowCount();i++) {
							try {
								...
								if(buscaIdgml.equalsIgnoreCase("GRM")) {
									...
									String metodov = Metodos_numericos.convierteAkilogramos(Double.parseDouble(def_tabla.getValueAt(i, 5).toString()));
									String resultado = Metodos_numericos.convierteAkilogramos((Double.parseDouble(metodov) *1000));
